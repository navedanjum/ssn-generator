const getCenturianDigit = (year = 2022): number => {
  if (year > 1857 && year < 2058) {
...
export const generateSSN = (dd: string, mm: string, yyyy: string): string => {
  const controlDigits = [4, 3, 2, 7, 6, 5, 4, 3, 2];
  const formattedDob = dd + mm + yyyy.substring(2);
  let cpr = '';
  let sum = 0;
  ...
  while (1) {
    cpr = '';
    const centurianDigit = getCenturianDigit(Number(yyyy));

    if (centurianDigit === -1) {
      console.log('CPR specification does not exist for the given birth year');
      throw new Error(
        'CPR specification does not exist for the given birth year'
      );
    }
    ...
    for (let i = 0; i < cpr.length; i++)
      sum += Number(cpr.charAt(i)) * controlDigits[i];
